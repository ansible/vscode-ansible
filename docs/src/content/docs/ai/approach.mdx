...
<CardGrid>
  <Card title="MCP Tools" icon="rocket">
    A standalone [MCP server](/vscode-ansible/ai/mcp-server/) with 22+ tools for collection
  ...
  <Card title="Skills" icon="document">
    [13 guided workflows](/vscode-ansible/ai/skills/) written as portable SKILL.md files.
...
|------------|---------|--------|---------|------------------|
| Language server (completions, diagnostics, hover) | Yes | Yes | Yes | [Standalone](/vscode-ansible/ai/language-server/) |
| MCP server auto-registration | `vscode.lm` API | `cursor.mcp` API | `~/.bob/settings/mcp.json` | [Manual config](/vscode-ansible/ai/connecting-agents/) |
...
<CardGrid>
  <Card title="Install the extension" icon="laptop">
    Works out of the box in [VS Code](/vscode-ansible/editor/vscode-extension/),
    [Cursor](/vscode-ansible/editor/cursor/), and [IBM Bob](/vscode-ansible/editor/ibm-bob/).
  ...
  <Card title="Connect a standalone agent" icon="setting">
    ...
    See [Connecting AI Agents](/vscode-ansible/ai/connecting-agents/) for setup.
  </Card>
</CardGrid>
