...
describe("SchemaValidator", () => {
  ...
  it("provides fallback range when node not found", () => {
  ...
  it("handles simple key comparison in findNode", () => {
    const doc = TextDocument.create(
      "file:///test.yml",
      "yaml",
      1,
  ...
  it("handles error path through scalar value", () => {
    // Schema expects nested object but YAML has scalar - tests findNode edge case
  ...
  it("handles validation error in array item", () => {
    ...
    const diags = validator.validate(doc, arraySchema);
    expect(diags).to.have.lengthOf(1);
    expect(diags[0].range.start.line).to.be.greaterThanOrEqual(2);
  });

  it("uses fallback range when node has no range info", () => {
  ...
  it("handles path through scalar triggering fallback", () => {
    // Schema expects nested object but value is scalar
    // Error path may try to navigate through scalar, triggering findNode return
    const nestedSchema: JSONSchema = {
      type: "object",
      properties: {
        config: {
