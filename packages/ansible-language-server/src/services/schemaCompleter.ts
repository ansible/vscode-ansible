...
import { parseDocument, isMap, isSeq, Pair, Scalar } from "yaml";
...
export class SchemaCompleter {
  complete(doc: TextDocument, pos: Position, schema: JSONSchema): CompletionItem[] {
  ...
  private findPath(node: unknown, offset: number, path: string[]): string[] {
    ...
    if (isMap(node)) {
      for (const pair of node.items) {
        ...
        const range = (p as unknown as { range?: [number, number, number] }).range;
        if (range && offset >= range[0] && offset <= range[2]) {
          const key = this.scalarValue(p.key);
          if (key) {
            const valRange = (p.value as unknown as { range?: [number, number, number] })?.range;
  ...
  private getSchemaAtPath(schema: JSONSchema, path: string[]): JSONSchema | undefined {
  ...
  private getExistingKeys(node: unknown, path: string[]): Set<string> {
    ...
    for (const key of path) {
      ...
      if (isMap(current)) {
        const pair = current.items.find((p) => this.scalarValue((p as Pair).key) === key);
        current = pair ? (pair as Pair).value : undefined;
    ...
    if (isMap(current)) {
      for (const pair of current.items) {
        const k = this.scalarValue((pair as Pair).key);
        if (k) keys.add(k);
      }
    }
    return keys;
