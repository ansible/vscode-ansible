...
import type { ExtensionSettingsWithDescriptionBase } from "@src/interfaces/extensionSettings";
...
export function generateSettingsDocs(outputPath: string) {
  ...
  fs.writeFileSync(outputPath, `${WARNING_IN_README}\n${output}`);

  console.log(`Settings documentation written to: ${outputPath}`);
}
...
function structureSettings(
  ...
  for (const k in settingsInDotNotation) {
    ...
    const arrayElementIndex = keyArray.indexOf("_array");
    const arrayElement =
...
function makeSettingsUnique(arrayObject: SettingEntry[]): SettingEntry[] {
