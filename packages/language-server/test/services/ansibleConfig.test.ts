...
describe('AnsibleConfig', () => {
    ...
    it('parses ansible-config dump and ansible --version output', async () => {
        ...
        expect(config.collectionPaths).toEqual(['/usr/share/ansible/collections']);
        expect(config.defaultHostList).toEqual(['/etc/ansible/hosts']);
        expect(config.moduleLocations).toContain('/home/user/.ansible/plugins/modules');
        expect(config.ansibleLocation).toBe('/usr/lib/python3/dist-packages/ansible');
    ...
    it('shows error message when command throws Error', async () => {
        ...
        expect(conn.window.showErrorMessage).toHaveBeenCalledWith('command not found');
