...
export class SettingsManager {
    ...
    private configurationChangeHandlers = new Map<string, () => void>();
    private documentSettings = new Map<string, Thenable<ExtensionSettings>>();
    ...
    readonly defaultSettingsWithDescription: ExtensionSettingsWithDescription = {
        ...
        validation: {
            ...
            lint: {
                ...
                autoFixOnSave: {
                    default: false,
                    description: 'Whether `ansible-lint --fix` should run automatically on save',
                },
            },
        },
    };
    ...
    public async handleConfigurationChanged(params: DidChangeConfigurationParams): Promise<void> {
        if (this.clientSupportsConfigRequests) {
            ...
            const ansibleSettings = (params.settings as { ansible?: ExtensionSettings }).ansible;
            if (ansibleSettings) {
                this.configurationChangeHandlers.forEach((h) => {
                    h();
                });
            }
            this.globalSettings = ansibleSettings ?? this.defaultSettings;
        }
    }
    ...
    private settingsAdjustment(
        ...
        for (const key in settingsObject) {
            ...
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                if (Object.hasOwn(value, 'default') && typeof value.default !== 'object') {
                    settingsObject[key] = value.default;
                } else {
                    this.settingsAdjustment(value);
                }
            }
        }
