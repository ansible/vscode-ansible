...
interface CodeClimateLocation {
    path: string;
    positions?: { begin: CodeClimatePosition };
    lines?: {
        begin: CodeClimatePosition | number;
        column?: number;
    };
}

interface CodeClimateItem {
    check_name: string;
    description?: string;
...
export class AnsibleLint {
    ...
    public async doValidate(textDocument: TextDocument): Promise<Map<string, Diagnostic[]>> {
        ...
        let ansibleLintConfigPath = /(?:^|\s)-c\s*(?<sep>[\s'"])(?<conf>.+?)(?:\k<sep>|$)/.exec(
            linterArguments,
        )?.groups?.conf;
        ...
        try {
            ...
            if (error instanceof Error) {
                this.connection.console.error(`Exception in AnsibleLint service: ${error.message}`);
            }
        }
    ...
    private processReport(result: string, workingDirectory: string): Map<string, Diagnostic[]> {
        ...
        try {
            const report = JSON.parse(result) as unknown;
            if (!Array.isArray(report)) return diagnostics;

            for (const rawItem of report) {
                if (!isCodeClimateItem(rawItem)) {
                    continue;
                }
                const item = rawItem;

                const hasPosition =
                    item.location.positions?.begin !== undefined ||
                    item.location.lines?.begin !== undefined;
                if (!hasPosition) continue;

                const beginLine = item.location.positions
                    ? item.location.positions.begin.line
                    : getLineNumber(item.location.lines?.begin);
                const beginColumn = item.location.positions
                    ? (item.location.positions.begin.column ?? 1)
                    : getColumnNumber(item.location.lines?.begin);
...
function isCodeClimateItem(value: unknown): value is CodeClimateItem {
    ...
    return (
        typeof record.check_name === 'string' &&
