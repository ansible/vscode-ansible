...
import type { PluginOption } from '@ansible/developer-services';
...
const tokenTypesLegend = new Map(tokenTypes.map((value, index) => [value, index]));

export const tokenModifiers = [SemanticTokenModifiers.definition] as const;
...
export async function doSemanticTokens(
    ...
    for (const yDoc of yDocuments) {
        if (yDoc.contents) {
            await markSemanticTokens([yDoc.contents], builder, document, collectionsService);
        }
    }
...
async function markSemanticTokens(
    ...
    if (isMap(node)) {
            ...
            if (isScalar(pair.key)) {
                ...
                if (isPlayParam(keyPath)) {
                    ...
                    if (isTaskKeyword(String(pair.key.value))) {
                        ...
                        if (pluginData) {
                            markNode(pair.key, SemanticTokenTypes.class, [], builder, document);
        ...
        for (const item of node.items) {
            if (isNode(item)) {
                await markSemanticTokens(path.concat(item), builder, document, collectionsService);
            }
        }
    }
}
...
function markModuleParameters(
    ...
    for (const moduleParamPair of moduleParamMap.items) {
        if (isScalar(moduleParamPair.key)) {
            ...
            markNode(moduleParamPair.key, SemanticTokenTypes.method, [], builder, document);
            if (option.type === 'dict' && option.suboptions && isMap(moduleParamPair.value)) {
                ...
                markAllNestedKeysAsOrdinary(moduleParamPair.value as Node, builder, document);
            }
...
function encodeTokenModifiers(modifiers: SemanticTokenModifiers[]): number {
    ...
    for (const modifier of modifiers) {
        const index = tokenModifiersLegend.get(modifier as (typeof tokenModifiers)[number]);
        if (index === undefined) {
