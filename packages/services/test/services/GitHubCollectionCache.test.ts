...
import { GitHubCollectionCache, type GitHubCollection } from '../../src/GitHubCollectionCache';
...
interface GitHubCollectionCachePrivate {
    _parseGalaxyYml(content: string, repoFullName: string, org: string): GitHubCollection | null;
    _formatAge(ms: number): string;
    _saveToDisk(org: string, cache: GitHubOrgCache): void;
    _scanOrg(org: string, token: string): Promise<GitHubCollection[]>;
    _scanOrgRepos(org: string, token: string): Promise<GitHubCollection[]>;
    _fetchCollectionMetadata(
        repoFullName: string,
        token: string,
        org: string,
...
`;
        ...
        const col = privateCache(svc)._parseGalaxyYml(yaml, 'demo_org/demo_repo', 'demo_org');
        expect(col).toEqual({
            namespace: 'demo_ns',
            name: 'demo_name',
            version: '2.0.0',
    ...
    it('_formatAge returns just now, minutes, hours, and days (private API)', () => {
        ...
        const fmt = privateCache(svc)._formatAge.bind(svc);
    ...
    it('refreshAll invokes refresh for every org (standalone no-ops)', async () => {
        ...
        expect(log).toHaveBeenCalledWith(
            expect.stringContaining('Cannot refresh a - not in VS Code context'),
        );
        expect(log).toHaveBeenCalledWith(
            expect.stringContaining('Cannot refresh b - not in VS Code context'),
        );
    });
...
`;
        ...
        expect(privateCache(svc)._parseGalaxyYml(yaml, 'o/r', 'o')).toBeNull();
        const yaml2 = `namespace: ns
name: n
description: [a, b]
`;

        const col = privateCache(svc)._parseGalaxyYml(yaml2, 'org/repo', 'org');
    ...
    it('_saveToDisk creates cache dir and writes file', () => {
        ...
        privateCache(svc)._saveToDisk('saveorg', cache);
        const written = JSON.parse(
            fs.readFileSync(path.join(cacheDir, 'github-saveorg.json'), 'utf-8'),
        ) as GitHubOrgCache;
    ...
    it('_saveToDisk handles write errors gracefully', () => {
        ...
        try {
            ...
            privateCache(svc)._saveToDisk('err', cache);
    ...
    it('_parseGalaxyYml returns null for non-object YAML', () => {
        ...
        expect(privateCache(svc)._parseGalaxyYml('just a string', 'o/r', 'o')).toBeNull();
    });
    ...
    it('_parseGalaxyYml returns null for invalid YAML', () => {
        ...
        expect(privateCache(svc)._parseGalaxyYml('{{invalid: yaml: [', 'o/r', 'o')).toBeNull();
    ...
    it('_parseGalaxyYml uses 0.0.0 when version is missing', () => {
        ...
        const col = privateCache(svc)._parseGalaxyYml(yml, 'o/r', 'o');
    ...
    it('_scanOrg uses code search API and fetches metadata', async () => {
        ...
        const galaxyYml = 'namespace: testns\nname: testcol\nversion: 2.0.0\ndescription: A test\n';
        ...
        const fetchMock = vi
            ...
            .mockResolvedValueOnce({ ok: true, text: () => Promise.resolve(galaxyYml) });
        vi.stubGlobal('fetch', fetchMock);

        const result = await privateCache(svc)._scanOrg('org', 'fake-token');
    ...
    it('_scanOrg falls back to repo scan when code search fails', async () => {
        ...
        const fetchMock = vi
            ...
            .mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve([{ full_name: 'org/repo', html_url: 'h' }]),
            })
            .mockResolvedValueOnce({ ok: true, text: () => Promise.resolve(galaxyYml) });
        vi.stubGlobal('fetch', fetchMock);

        const result = await privateCache(svc)._scanOrg('org', 'token');
    ...
    it('_scanOrg returns empty when no galaxy.yml files found', async () => {
        ...
        const result = await privateCache(svc)._scanOrg('emptyorg', 'token');
    ...
    it('_scanOrg handles fetch errors gracefully', async () => {
        ...
        const result = await privateCache(svc)._scanOrg('errorg', 'token');
        expect(result).toEqual([]);
    ...
    it('_scanOrgRepos paginates and stops when page is short', async () => {
        ...
        const fetchMock = vi
            ...
            .mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve([{ full_name: 'org/r1', html_url: 'h' }]),
            })
            .mockResolvedValueOnce({ ok: true, text: () => Promise.resolve(galaxyYml) });
        vi.stubGlobal('fetch', fetchMock);

        const result = await privateCache(svc)._scanOrgRepos('org', 'token');
        expect(result).toHaveLength(1);
    ...
    it('_scanOrgRepos skips repos without galaxy.yml', async () => {
        ...
        const fetchMock = vi
            ...
            .mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve([{ full_name: 'org/nope', html_url: 'h' }]),
            })
            .mockResolvedValueOnce({ ok: false })
            .mockResolvedValueOnce({ ok: false })
            .mockResolvedValueOnce({ ok: false })
    ...
    it('_fetchCollectionMetadata tries galaxy.yml then galaxy.yaml with main and HEAD branches', async () => {
        ...
        const fetchMock = vi
            ...
            .mockResolvedValueOnce({ ok: false })
            .mockResolvedValueOnce({ ok: false })
            .mockResolvedValueOnce({ ok: true, text: () => Promise.resolve(galaxyYaml) });
        vi.stubGlobal('fetch', fetchMock);

        const result = await privateCache(svc)._fetchCollectionMetadata('org/repo', 'token', 'org');
