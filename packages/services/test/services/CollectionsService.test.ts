...
import { CollectionsService } from '../../src/CollectionsService';
...
describe('CollectionsService', () => {
    ...
    it('installCollection calls ade and forceRefreshes on success', async () => {
        const svc = CollectionsService.getInstance();
        await svc.forceRefresh();
        runToolMock.mockImplementation((toolName: string, args: string[]) => {
    ...
    it('persists collections-metadata.json cache with plugin docs after forceRefresh', async () => {
        ...
        const raw = JSON.parse(fs.readFileSync(cacheFile, 'utf8')) as {
            ...
            pluginDocs: Record<string, { doc: { short_description: string } }>;
        };
        expect(Array.isArray(raw.collections)).toBe(true);
        expect(raw.collections.some((c) => c.name === 'ansible.builtin')).toBe(true);
        expect(raw.pluginDocs).toBeDefined();
        expect(raw.pluginDocs['ansible.builtin.copy:module']).toBeDefined();
        expect(raw.pluginDocs['ansible.builtin.copy:module'].doc.short_description).toBe(
    ...
    it('refresh loads from disk cache immediately when file exists', async () => {
        ...
        await vi.waitUntil(() => runToolMock.mock.calls.length >= callsBefore + 2, {
            timeout: 3000,
        });
    });
