...
import { GalaxyCollectionCache } from '../../src/GalaxyCollectionCache';
...
describe('GalaxyCollectionCache', () => {
    ...
    it('search filters by name, namespace, and fqcn; empty query returns top 100', async () => {
        ...
        expect(
            filtered.every(
                (c) => c.name.includes('coll5') || `${c.namespace}.${c.name}`.includes('coll5'),
            ),
        ).toBe(true);
    ...
    it('ensureLoaded is a no-op on second call when already loaded', async () => {
        ...
        const cache = {
            timestamp: Date.now(),
            collections: [
                { namespace: 'a', name: 'b', version: '1', deprecated: false, downloadCount: 1 },
            ],
        };
        fs.writeFileSync(
            path.join(storage, 'galaxy-collections-cache.json'),
    ...
    it('fetches multiple pages when API returns relative next link', async () => {
        ...
        let call = 0;
        httpsGetMock.mockImplementation(
            (url: unknown, _options: unknown, cb: (res: EventEmitter) => void) => {
    ...
    it('getProgress and onDidUpdateProgress fire during fetch', async () => {
        ...
        const sub = svc.onDidUpdateProgress((e: { loaded: number; total: number }) =>
            progress.push(e),
        );
    ...
    it('_loadFromFileCache returns false when cacheFilePath is undefined', async () => {
        ...
        const spy = vi
            .spyOn(
                GalaxyCollectionCache.prototype as unknown as {
                    _cacheFilePath: string | undefined;
                },
                '_cacheFilePath',
                'get',
            )
        ...
        const result = await (
            svc as unknown as { _loadFromFileCache: () => Promise<boolean> }
        )._loadFromFileCache();
        spy.mockRestore();
    ...
    it('_loadCollections skips API when already loaded and not force refresh', async () => {
        ...
        await svc.ensureLoaded();
        await (
            svc as unknown as { _loadCollections: (force: boolean) => Promise<void> }
        )._loadCollections(false);
        expect(httpsGetMock).not.toHaveBeenCalled();
    });

    it('getCacheAge returns singular day form', () => {
        const svc = GalaxyCollectionCache.getInstance();
        (svc as unknown as { _cacheTimestamp: number })._cacheTimestamp =
            Date.now() - 25 * 60 * 60 * 1000;
        expect(svc.getCacheAge()).toBe('1 day ago');
    });

    it('getCacheAge returns singular hour form', () => {
        const svc = GalaxyCollectionCache.getInstance();
        (svc as unknown as { _cacheTimestamp: number })._cacheTimestamp =
            Date.now() - 90 * 60 * 1000;
                ...
                const body =
                    call++ === 0
                        ? {
                              meta: { count: 2 },
                              links: {
                                  next: 'https://galaxy.ansible.com/api/v3/collections/?page=2',
                              },
                              data: [
                                  {
                                      namespace: 'p1',
    ...
    it('_fetchPage handles request timeout', async () => {
        ...
        const p = (
            svc as unknown as { _fetchPage: (u: string, r?: number) => Promise<unknown> }
        )._fetchPage('https://example.com', 1);
    ...
    it('_fetchPage follows redirects with relative URL', async () => {
        ...
        const result = await (
            svc as unknown as {
                _fetchPage: (u: string, r?: number) => Promise<{ data: unknown[] }>;
            }
        )._fetchPage('https://galaxy.ansible.com/api', 3);
    ...
    it('_fetchPage retries on response stream error then succeeds', async () => {
        ...
        )._fetchPage('https://example.com', 2);
        await vi.advanceTimersByTimeAsync(2000);
