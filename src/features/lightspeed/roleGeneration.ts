      ...
      case "saveRole": {
        ...
        const promises = files.map(async (f) => {
          ...
          if (await fileExists(fileUri)) {
            addLog(`<strong>File already exists</strong> (${fileUri})!`);
            return;
          }
...
export async function getWebviewContent(webview: Webview, extensionUri: Uri) {
  ...
  webview.postMessage({
    ...
    collectionList: (await getCollectionsFromWorkspace()).map((i) =>
      i.asString(),
    ),
  });
...
<body>
    <div class="playbookGeneration">
        ...
        <div id="roleNameContainer">
            ...
            <p id="roleAlreadyExists">The role directory already exists!</p>
          </div>
        ...
        <div class="saveRoleContainer">
          <ul id="saveRoleLogArea"  placeholder="Output of the command execution"
           readonly></ul>
          <vscode-button class="biggerButton" id="generateRoleButton">
              Save files
          </vscode-button>
