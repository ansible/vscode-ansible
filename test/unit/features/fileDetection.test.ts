...
describe('parseModelineLanguage', () => {
    ...
    it('allows up to 8 characters before code:', () => {
        expect(parseModelineLanguage('12345678code: language=ansible')).toBe('ansible');
        expect(parseModelineLanguage('123456789code: language=ansible')).toBeUndefined();
    });

    it('handles multiple options, takes the last language match', () => {
        expect(parseModelineLanguage('# code: lang=yaml language=ansible')).toBe('ansible');
    });
});
...
describe('searchModelineLanguage', () => {
    ...
    it('works with very short files (fewer than 10 lines)', () => {
        expect(searchModelineLanguage('# code: language=ansible')).toBe('ansible');
        expect(searchModelineLanguage('---\n# code: language=ansible')).toBe('ansible');
    });
});
