...
describe('parseVaultIdentities', () => {
    ...
    it('parses multiple identities', () => {
        expect(parseVaultIdentities('dev@/path/dev, prod@/path/prod')).toEqual(['dev', 'prod']);
    });

    it('trims whitespace around identities', () => {
        expect(parseVaultIdentities('  dev@script ,  prod@script  ')).toEqual(['dev', 'prod']);
    });
...
describe('getVaultConfig', () => {
    ...
    it('returns config from ANSIBLE_VAULT_IDENTITY_LIST env var', async () => {
        ...
        if (!cfg) throw new Error('expected vault config');
