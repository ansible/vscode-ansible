...
describe("getRoleNamePathFromFilePath", function () {
  ...
  it("should return an empty string when the file path does not contain 'roles'", function () {
    const testPath = path.join("vscode-ansible", "playbooks", "main.yml");
    const result = getRoleNamePathFromFilePath(testPath);
    expect(result).to.equal("");
  });

  it("should return the correct role name path when the file path contains multiple 'roles'", function () {
    const testPath = path.join(
      "Users",
      "ansible",
      "Desktop",
