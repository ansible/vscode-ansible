...
# create a vpc & create vpc_id var     `;
    ...
    it("should not make request when prompt line start with '# ' and end with ' &' for tasks", function () {
      const promptContent = `---
# Create a key-pair called lightspeed-key-pair &
# create a vpc & create vpc_id var &
# create a security group that allows SSH & create subnet with 10.0.1.0/24 cidr &
# create an internet gateway & create a route table &`;
    ...
    it("should not make request when prompt line start with '# ' and end with ' &' for playbook", function () {
      const promptContent = `---
- name: Testing playbook
  hosts: all
  tasks:
    ...
    it("should allow request when prompt line start with '# ' and does not end with ' &' for tasks", function () {
      const promptContent = `---
# Create a key-pair called lightspeed-key-pair &
# create a vpc & create vpc_id var &
# create a security group that allows SSH & create subnet with 10.0.1.0/24 cidr &
# create an internet gateway & create a route table`;
    ...
    it("should allow request when prompt line start with '# ' and does not end with ' &' for playbook", function () {
      const promptContent = `---
- name: Testing playbook
  hosts: all
  tasks:
