...
suite('Ansible Environments Extension', () => {
    ...
    test('contributes ansible-environments view container', () => {
        const ext = vscode.extensions.getExtension(EXTENSION_ID);
    ...
    test('configuration defaults are correct', () => {
        ...
        assert.ok(
            orgs.includes('ansible-collections'),
            "default orgs should include 'ansible-collections'",
        );
    });

    test('refresh commands are executable without error', async () => {
        const prefixes = ['ansibleDevToolsPackages', 'ansibleDevToolsCollections'];
        const commands = await vscode.commands.getCommands(true);
    ...
    test('contributes MCP server tools via chat participant', () => {
        ...
        if (languageModelTools) {
            ...
            assert.ok(
                languageModelTools.length > 0,
                'should have at least one language model tool',
            );
