...
describe('Ansible Language Server e2e', () => {
    it('should detect .ansible.yml file as ansible language', async () => {
        ...
        const fixtureUri = path.resolve(process.cwd(), 'test', 'ui', 'fixtures', PLAYBOOK_FILENAME);
        await browser.executeWorkbench(async (vscode: typeof VsCode, uri: string) => {
        ...
        const languageId = await browser.executeWorkbench((vscode: typeof VsCode) => {
    ...
    it('should start the Ansible Language Server', async () => {
        const outputText: string = await browser.executeWorkbench((vscode: typeof VsCode) => {
    ...
    it('should verify the editor has ansible-specific semantic highlighting', async () => {
        ...
        const hasTokenization: boolean = await browser.executeWorkbench((vscode: typeof VsCode) => {
            const editor = vscode.window.activeTextEditor;
