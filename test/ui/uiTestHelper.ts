  ...
  const condition = async () => {
    try {
      ...
      try {
        ...
        console.log("No 'webview ready' iframe located in this poll attempt.");
        return false;
      }
      ...
      for (let i = iframes.length - 1; i >= 0; i--) {
        try {
          ...
          if (activeFrames.length === 0) {
            console.log(`No 'active-frame' found in iframe ${iframeName}`);
          }
          ...
          console.log(`locator=${locator} not found in iframe ${iframeName}`);
        } catch (error) {
