...
async function resolveChromiumMajor(
  ...
  for (const vscodeVersion of vscodeVersions) {
    ...
    try {
      ...
      errors.push(
        `${vscodeVersion}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }

  throw new Error(
    `Could not determine Chromium major version. Attempts:\n${errors.join("\n")}`,
  );
}
...
async function getChromedriverVersion(chromiumMajor: string): Promise<string> {
  ...
  if (version === undefined || version.length === 0) {
    throw new Error(
      `No chromedriver available for Chromium milestone ${chromiumMajor}`,
    );
  }
  return version;
}
...
function driverPaths(
  ...
  const driverDir = join(
    cacheDir,
    "chromedriver",
    `${platform}-${chromedriverVersion}`,
  );
  const driverBin = join(driverDir, `chromedriver-${folder}`, "chromedriver");
...
async function ensureChromedriverBinary(
  ...
  const { driverDir, driverBin } = driverPaths(
    CACHE_DIR,
    platform,
    chromedriverVersion,
    folder,
  ...
  if (existsSync(driverBin)) {
    ...
    console.log(
      `Chromedriver ${chromedriverVersion} already available at ${driverBin}`,
    );
    return;
  }

  console.log(
    `Downloading chromedriver ${chromedriverVersion} for ${platform}/${folder}...`,
  );
  mkdirSync(driverDir, { recursive: true });

  const downloadUrl = DOWNLOAD_URL.replace(
    "{version}",
    chromedriverVersion,
  ...
  try {
    ...
    chmodSync(driverBin, 0o755);
    console.log(
      `Chromedriver ${chromedriverVersion} installed at ${driverBin}`,
    );
  } catch (err) {
...
async function main(): Promise<void> {
  ...
  const vscodeForCache =
    pickInstalledVscodeVersion(installedVscode, vscodeVersions) ??
    vscodeVersions[0];
